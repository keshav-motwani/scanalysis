...
```{r ambient_filter}
cell_dataset = map2(dataset, ambient_rna_filters, ~ .x[,  .y$empty_drops == TRUE])
```
...
```{r annotate}
cell_dataset = map(cell_dataset, ~ .x %>%
...
```{r filter_qc}
total_umi_filters = map(cell_dataset, ~ isOutlier(colData(.x)$total_umi,
                                                   nmads = 1,
                                                   type = "both",
                                                   log = TRUE))

n_genes_expr_filters = map(cell_dataset, ~ !isOutlier(colData(.x)$n_genes_expr,
                                                      nmads = 1,
                                                      type = "both",
                                                      log = TRUE))

pct_mito_filters = map(cell_dataset, ~ !isOutlier(colData(.x)$pct_mito,
                                                  nmads = 2,
                                                  type = "higher",
                                                  log = FALSE))
```
...
```{r plot_qc, fig.height=6, fig.width=8}
plot_qc_distribution(cell_dataset, x_filters = total_umi_filters, x = "total_umi", color = "pct_mito", facet_columns = ".sample")

plot_qc_distribution(cell_dataset, x_filters = n_genes_expr_filters, x = "n_genes_expr", color = "pct_mito", facet_columns = ".sample")

plot_qc_distribution(cell_dataset, x_filters = pct_mito_filters, x = "pct_mito", color = "total_umi", facet_columns = ".sample", x_log = FALSE)

plot_qc_scatterplot(cell_dataset, x_filters = total_umi_filters, y_filters = n_genes_expr_filters, x = "total_umi", y = "n_genes_expr", color = "pct_mito", facet_columns = ".sample",  facet_type = "wrap")
```