...
#' @examples
#' NULL
compute_evenness_profile_matrix = function(sce_list, group_by = c(), min_alpha = 0, max_alpha = 10, alpha_steps = 51) {

  if (is.null(names(sce_list))) names(sce_list) = paste0("sample_", 1:length(sce_list))

  stopifnot(all(purrr::map_lgl(sce_list, ~ "clonotype" %in% colnames(colData(.x)))))

  data = purrr::map2_dfr(sce_list,
                         names(sce_list),
...
#' @examples
#' NULL
compute_evenness_profile_long = function(sce_list, group_by = c(), min_alpha = 0, max_alpha = 10, alpha_steps = 51) {
  ...
  evenness_profile = dplyr::as_tibble(evenness_profile_matrix) %>%
    ...
    tidyr::pivot_longer(cols = -c("group", colnames(group_annotations)), names_to = "alpha", values_to = "evenness") %>%
...
compute_evenness_profile = function(count_vectors, min_alpha, max_alpha, alpha_steps) {
  alphas = seq(min_alpha, max_alpha, length.out = alpha_steps)
  matrix = sapply(
    1:length(count_vectors),
    function(x)
      sapply(
        1:length(alphas),
        function(i)
          exp(renyi_entropy(count_vectors[[x]]/sum(count_vectors[[x]]), alpha = alphas[i]))
